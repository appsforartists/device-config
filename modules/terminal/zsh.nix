{
  ...
  config = {
    ...
    in {
      ...
      # You have to manually `sudo chsh -s /bin/zsh $USER` to change shells.
      # The Nix-managed version of zsh isn't in `/etc/shells`, and it's
      # probably not worth it to force it.
      package = lib.mkIf pkgs.stdenv.isDarwin pkgs.emptyDirectory; # doesn't support `null`
